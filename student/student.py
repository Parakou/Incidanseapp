...
def oldstudent():
    if "sort" in request.form:
        buttonvalue = request.form['sort']
        if buttonvalue == 'nom':
            eleve = Eleve.query.filter_by(eleve_actif="0").order_by("eleve_nom").all()
            return render_template('student/student.html', eleve=eleve)

        if buttonvalue == 'prenom':
            eleve = Eleve.query.filter_by(eleve_actif="0").order_by("eleve_prenom").all()
            return render_template('student/student.html', eleve=eleve)

        if buttonvalue == 'age':
            eleve = Eleve.query.filter_by(eleve_actif="0").order_by("eleve_age").all()
            return render_template('student/student.html', eleve=eleve)

        if buttonvalue == 'heuredecours':
            eleve = Eleve.query.filter_by(eleve_actif="0").order_by("eleve_heure").all()
            return render_template('student/student.html', eleve=eleve)
